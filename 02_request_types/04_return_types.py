...
@app.post("/task/dummy")
async def create_task(task: Task) -> Task:  # Does not do anything
    return task


@app.post("/task/")
async def create_task(task: Task) -> dict:  # Puts the task in queue
    todo.appendleft(task)
    return {'message': 'ok'}


@app.get("/task/")
async def get_task(task: Task) -> dict:  # Consume the task from the queue
    if len(todo) != 0:
